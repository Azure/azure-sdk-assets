...
type CertificateProfilesServer struct {
	...
	RevokeCertificates func(ctx context.Context, resourceGroupName string, accountName string, profileName string, body armcodesigning.RevokeCertificateList, options *armcodesigning.CertificateProfilesClientRevokeCertificatesOptions) (resp azfake.Responder[armcodesigning.CertificateProfilesClientRevokeCertificatesResponse], errResp azfake.ErrorResponder)
}
...
func (c *CertificateProfilesServerTransport) dispatchRevokeCertificates(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armcodesigning.RevokeCertificateList](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
