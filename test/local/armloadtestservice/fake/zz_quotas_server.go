...
// QuotasServer is a fake server for instances of the armloadtestservice.QuotasClient type.
type QuotasServer struct {
	// CheckAvailability is the fake for method QuotasClient.CheckAvailability
	// HTTP status codes to indicate success: http.StatusOK
	CheckAvailability func(ctx context.Context, location string, quotaBucketName string, quotaBucketRequest armloadtestservice.QuotaBucketRequest, options *armloadtestservice.QuotasClientCheckAvailabilityOptions) (resp azfake.Responder[armloadtestservice.QuotasClientCheckAvailabilityResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method QuotasClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, location string, quotaBucketName string, options *armloadtestservice.QuotasClientGetOptions) (resp azfake.Responder[armloadtestservice.QuotasClientGetResponse], errResp azfake.ErrorResponder)

	// NewListPager is the fake for method QuotasClient.NewListPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListPager func(location string, options *armloadtestservice.QuotasClientListOptions) (resp azfake.PagerResponder[armloadtestservice.QuotasClientListResponse])
}

// NewQuotasServerTransport creates a new instance of QuotasServerTransport with the provided implementation.
// The returned QuotasServerTransport instance is connected to an instance of armloadtestservice.QuotasClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewQuotasServerTransport(srv *QuotasServer) *QuotasServerTransport {
	return &QuotasServerTransport{
		srv:          srv,
		newListPager: newTracker[azfake.PagerResponder[armloadtestservice.QuotasClientListResponse]](),
	}
}

// QuotasServerTransport connects instances of armloadtestservice.QuotasClient to instances of QuotasServer.
// Don't use this type directly, use NewQuotasServerTransport instead.
type QuotasServerTransport struct {
	srv          *QuotasServer
	newListPager *tracker[azfake.PagerResponder[armloadtestservice.QuotasClientListResponse]]
}
...
func (q *QuotasServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armloadtestservice.QuotasClientListResponse, createLink func() string) {
