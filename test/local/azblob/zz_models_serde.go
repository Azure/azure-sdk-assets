...
// MarshalXML implements the xml.Marshaller interface for type Properties.
func (p Properties) MarshalXML(enc *xml.Encoder, start xml.StartElement) error {
	...
	aux := &struct {
		*alias
		...
		ImmutabilityPolicyExpiresOn: (*datetime.RFC7231)(p.ImmutabilityPolicyExpiresOn),
...
// UnmarshalXML implements the xml.Unmarshaller interface for type Properties.
func (p *Properties) UnmarshalXML(dec *xml.Decoder, start xml.StartElement) error {
	type alias Properties
	aux := &struct {
		*alias
		AccessTierChangeTime        *datetime.RFC7231 `xml:"AccessTierChangeTime"`
